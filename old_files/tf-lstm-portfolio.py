...
def model(x, sequence_length, batch_size,hidden_size_f1, hidden_size_ff, hidden_size_lstm):
	...
	lstm_input = tf.layers.dense(x,hidden_size_f1,activation = tf.nn.elu,use_bias = True,kernel_initializer = tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),kernel_regularizer = tf.contrib.layers.l2_regularizer)
...
def main():
	...
	hidden_size_f1 = 350 # hidden size of first ff connected layer.
	input_size = 295*4 # size of the word embedding or similar 
	learning_rate = 1e-2
	...
	pred_return = 1-tf.exp(y)# = (y-x)/x , profit/loss
	total_return = tf.multiply(portfolio, pred_return)
	days_return = tf.reduce_sum(tf.reduce_mean(total_return[:,-1,:], axis = 0))
	...
	for epoch in xrange(epochs):
		...
		print days_return.eval(feed_dict={x: test_X.astype(np.float32), y: test_Y.astype(np.float32)})
