...
n_train = -900 #number of training samples
n_test = int(n_train*(0.1)) #number of test samples
output_size = 319 #number of stocks
reg_str = 1.0 # regularization strength 
filter_conv1 = 3 # number of filters in 1st conv layer
filter_conv2 = 2 # number of filters in 2nd conv layer
filter_conv3 = 1 # number of filters in 3rd conv layer
kernel_conv1 = [21,1]  # shape of filters in 1st conv layer
kernel_conv2 = [14,1] # shape of filters in 2nd conv layer
kernel_conv3 = [7,1] # shape of filters in 3rd conv layer
hidden_size_lstm = 319 # 
batch_size = 30
sequence_length = 30 
adam = optimizers.Adam(lr = 1e-1, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
...
# output_conv1 = Conv2D(data_format="channels_last",filters = filter_conv1,kernel_regularizer = keras.regularizers.l2(reg_str),padding = "same",kernel_size = kernel_conv1,activation = 'elu',bias_initializer=Constant(.3))(data)

output_conv2 = Conv2D(data_format="channels_last",filters = filter_conv2,kernel_regularizer = keras.regularizers.l2(reg_str),padding = "same",kernel_size = kernel_conv2,activation = 'relu',bias_initializer=Constant(.3))(data)

output_conv3 = Conv2D(data_format="channels_last",filters = filter_conv3,kernel_regularizer = keras.regularizers.l2(reg_str),padding = "same",kernel_size = kernel_conv3,activation = 'relu',bias_initializer=Constant(.3))(output_conv2)


output_conv3 = Reshape([sequence_length,output_size])(output_conv3)
...
for k in range(10,30):
	...
	output_lstm = LSTM(hidden_size_lstm,recurrent_dropout=0.2, dropout=0.4, return_sequences=True, name = 'lstm')(output_conv3)

	output_lstm_d = Dropout(.4)(output_lstm)

	output_dense = Dense(output_size,activation = 'relu',kernel_regularizer = keras.regularizers.l2(1.0),bias_initializer=Constant(.3))(output_lstm)

	portfolio = Norm(axis=-1,name='portfolio')(output_dense)
	l = k+1
